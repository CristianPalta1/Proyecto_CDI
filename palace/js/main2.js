...
var fecha_fin2x1_es = '2021-06-15';
var setDate = new Date();
var setDate2 = setDate.getFullYear() + "-" + (setDate.getMonth() + 1) + "-" + setDate.getDate();
var setDate3 = new Date("'" + setDate2 + "'");
var setDate5 = new Date("'" + setDate2 + "'");
setDate3.setDate(setDate3.getDate() + 5);
var setDate4 = setDate3.getFullYear() + "-" + (setDate3.getMonth() + 1) + "-" + setDate3.getDate();
var getdate = 15; //Variable que define el día seleccionado por defecto del booking
...
var alturabooking = 0
var afiliate = jQuery("section.affiliate .referred-block")
...
/*Instrucción para mantener oculto el checkout en las LP 2x1 de acuerdo al hotel seleccionado*/
if (jQuery('.landing-2-1 .webform-submission-form').length > 0) {
  jQuery(document).ajaxComplete(function (event, xhr, settings) {
    ...
    if (retorsSelected2x1 == "le blanc spa resort cancun" || retorsSelected2x1 == "le blanc spa resort los cabos" || retorsSelected2x1 == "the grand at moon palace cancun" || retorsSelected2x1 == "sun palace") {
      jQuery('.checkout').val("");
      jQuery(".content-checkout").hide();
    } else {
      jQuery(".content-checkout").show();
    }
    jQuery(".landing-2-1 select.booking-hotel").change(function () {
      ...
      if (retorsSelected2x1 == "le blanc spa resort cancun" || retorsSelected2x1 == "le blanc spa resort los cabos" || retorsSelected2x1 == "the grand at moon palace cancun" || retorsSelected2x1 == "sun palace") {
        ...
        jQuery(".content-checkout").show();
        jQuery('.date-check .checkout').datepicker('setDate', setDate4);
      }
    });
  });
}
    ...
    if (jQuery('section.main_content_banner .video iframe').length != 0) {
      ...
      for (var i = 0; i < iframeBanner.length; i++) {
        if (jQuery(iframeBanner[i]).parent().attr('data-platform') == 'vimeo') {
          ...
          postMessages(jQuery(iframeBanner[i]).attr('id'), 'pauseVideo');

          jQuery(iframeBanner[i]).parent().find('.volumeVideo').removeClass('activado');
          jQuery(iframeBanner[i]).parent().removeClass('on');
          postMessages(jQuery(iframeBanner[i]).attr('id'), 'mute');
        }
      ...
      if (jQuery(".main_content_banner .owl-item").eq(event.item.index).find('.video').attr('data-platform') == 'vimeo') {
        ...
        if (jQuery(".main_content_banner .owl-item").eq(event.item.index).find('.video').length != 0) {
          ...
          postMessages(iframeyou, 'playVideo');
          postMessages(iframeyou, 'mute');
        }
...
function postMessages(frame_id, func, args) {
...
function centravideo() {
  ...
  if (afiliate.length > 0) {
    this.height_afiliate = afiliate.outerHeight() + 100;
    //console.log("altura Afiliate" + this.height_afiliate);
/*     afiliate.appendTo(jQuery("section.main_content_banner .content"));
 */  } else {
    this.height_afiliate = 0;
  }
  var widthWindow = jQuery(window).width();
  var height_widthWindow = 38.02 * widthWindow / 100;
  if (height_pictures == undefined || height_pictures < 400 && screen_width > 1200) {

    height_pictures = height_widthWindow + height_afiliate;
    //console.log("total banner" + height_pictures)

  } else if (screen_width <= 1200) {
    height_pictures = "auto"
  } else {
    height_pictures = height_pictures + height_afiliate;
  }
  ...
  jQuery("section.accommodations_galery .content article.virtual_tour").click(function () {
    ...
    if (tourid != undefined) {
      jQuery("section.main_lbox").fadeIn(500, function () {
        jQuery("section.main_lbox .lbox .item").html('<iframe title="video" class="lazyload" data-src="' + tourid + '"></iframe>');
      });
    }
  })
}
...
/* SEARCH FUNCTIONS */
function despliegue() {
  jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search .input-group input.form-control').keypress(function (e) {
    if (e.which == "13") {
      jQuery(document).ajaxComplete(function (event, xhr, settings) {
        jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search').css("display", "block");
        jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search .item-list').fadeIn(300).css("display", "block");
        jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search .pager-nav ').fadeIn(300).css("display", "block");
...
function ocultar() {
  jQuery(".dropdown_search").hide("slow");
  jQuery(".search-button").removeClass("pushed");
...
function down_search() {
  window.addEventListener('click', function (e) {
    var contenedor = jQuery(".searchContainer");
  ...
  jQuery('header nav.navbar-fixed-top .container .navbar-collapsed a.search-button').click(function (e) {
    jQuery('div.searchContainer .block_img figure p').text(textoBuscador);
    jQuery(".dropdown_search").toggle("slow", function () {
      despliegue();
    });
    jQuery(".search-button").toggleClass("pushed");
    jQuery("header").toggleClass("searching");
  });

  jQuery('header nav.navbar-fixed-top .container .navbar-collapsed #search-close').click(function (e) {
    jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .input-group input.form-text').val('');
    jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search .item-list').fadeOut(300).css("display", "none");
    jQuery('header nav.navbar-fixed-top .container .navbar-collapsed .navbar-top-search .pager-nav').fadeOut(300).css("display", "none");

  });

}

/* FIN SEARCH FUNCTIONS */
...
/*Insctructive Pop Up en Bucket list y Big Deals*/
function instructive_bucket() {
  jQuery('section.step-2 .main_instructive .instructive .close').click(function () {
    jQuery("section.main_instructive").fadeOut(500);
  })
  /* jQuery('div.affiliate .open').click(function () {
    jQuery("div.purley_icons section.main_instructive").fadeIn(500);
  }) */
  window.addEventListener('click', function (e) {
...
function popUpBucket() {
  jQuery("section.step-4 .step-4__activities .itemAct-1 picture").click(function () {
    jQuery("section.step-4 .step-4__activities .itemAct-1 .popUpactivities").fadeIn(500);
  })
  jQuery("section.step-4 .step-4__activities .itemAct-2 picture").click(function () {
    jQuery("section.step-4 .step-4__activities .itemAct-2 .popUpactivities").fadeIn(500);
  })
  jQuery("section.step-4 .step-4__activities .itemAct-3 picture").click(function () {
    jQuery("section.step-4 .step-4__activities .itemAct-3 .popUpactivities").fadeIn(500);
  })
  jQuery("section.step-4 .step-4__activities .itemAct-1 .close").click(function () {

    jQuery("section.step-4 .step-4__activities .itemAct-1 .popUpactivities").fadeOut(500);
  })
  jQuery("section.step-4 .step-4__activities .itemAct-2 .close").click(function () {
    jQuery("section.step-4 .step-4__activities .itemAct-2 .popUpactivities").fadeOut(500);
  })
  jQuery("section.step-4 .step-4__activities .itemAct-3 .close").click(function () {
    jQuery("section.step-4 .step-4__activities .itemAct-3 .popUpactivities").fadeOut(500);
  })
}


function galery_tabs() {
  jQuery("section.galery_tabs .content .content_tabs .box_tab.galery_photos .photo_dreamart").mouseenter(function () {
    jQuery(this).find("img").css("transform", "scale(1.2)")
  });

  jQuery("section.galery_tabs .content .content_tabs .box_tab.galery_photos .photo_dreamart").mouseleave(function () {
  ...
  jQuery("section.galery_tabs .content .content_tabs .box_tab.galery_videos .video").click(function () {
    ...
    if (videoid != undefined) {
      jQuery("section.main_lbox").fadeIn(500, function () {
        jQuery("section.main_lbox .lbox .item").html('<iframe title="video" class="lazyload" data-src="' + videoid + '" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe><script src="https://player.vimeo.com/api/player.js"></script>');
      });
    }
  })
...
function top_booking() {
  ...
  if (afiliate.length > 0) {
    //console.log("altura 1 main" + alturabooking)
    alturabooking = alturabooking + afiliate.outerHeight() + 80
    //console.log("mover booking " + alturabooking + " altura afiliate 2 " + afiliate.outerHeight())
...
function booking() {
  ...
  jQuery("#checkin_date_r").attr('readonly', 'readonly');
  jQuery("#checkout_date_r").attr('readonly', 'readonly');
  jQuery("#checkin_date_rf").attr('readonly', 'readonly');
  jQuery("#checkout_date_rf").attr('readonly', 'readonly');
  jQuery("#checkin_date_r_mex_b").attr('readonly', 'readonly');
  jQuery("#checkout_date_r_mex_b").attr('readonly', 'readonly');

  /* función que permmite adaptar la fecha del Check Out cuando se acerca a días desabilitados */
  function dateCheckOut(fecha, dias) {
    var fechaOut = new Date(fecha);
    var ajusteDias = dias;
    if (FechasDeshabilitadas.length != 0) {
      fechaOut.setDate(fechaOut.getDate() + dias);
      for (var j = 0; j < dias; j++) {
        for (var i = 0; i < FechasDeshabilitadas.length; i++) {
          var datetmp = new Date('' + FechasDeshabilitadas[i] + '');

          if (fechaOut.getTime() == datetmp.getTime()) {
            ajusteDias--;
            fechaOut.setDate(fechaOut.getDate() - 1);
            i = FechasDeshabilitadas.length;
          }
        }
      }
      dias = ajusteDias;
  ...
  /* función que permite adaptar la fecha del Check in en caso de coincidir con las fechas bloqueadas */
  function dateCheckIn(fecha) {
    var fechaIn = new Date(fecha);
    var ajusteDias = 0;
    var validacion = false;
    if (FechasDeshabilitadas.length != 0) {
      fechaIn.setDate(fechaIn.getDate() + ajusteDias);
      for (var j = 0; j <= FechasDeshabilitadas.length; j++) {
        validacion = false;
        for (var i = 0; i < FechasDeshabilitadas.length; i++) {
          var datetmp = new Date('' + FechasDeshabilitadas[i] + '');

          if (fechaIn.getTime() == datetmp.getTime()) {
            ajusteDias++;
  ...
  jQuery('input.form-control.date').change(function (ev) {
    ...
    if (cual == 'checkin_date_r') {
      ...
      //var temporal =  dateCheckIn(dater);
      //console.log("++++++++ "+temporal);
      //date_check_in.setDate(date_check_in.getDate() + (dateCheckIn(dater)));
      //jQuery('#checkin_date_r').datepicker('setStartDate', dater);
      //jQuery("#checkin_date_r").datepicker({ dateFormat: "mm/dd/yy" }).datepicker("setDate", date_check_in);


      date_check_out.setDate(date_check_out.getDate() + (dateCheckOut(dater, 5)));
      //console.log("fechas deshabilitadas "+FechasDeshabilitadas);

      jQuery('#checkin_date_r').datepicker('setDatesDisabled', FechasDeshabilitadas);
      jQuery('#checkout_date_r').datepicker('setDatesDisabled', FechasDeshabilitadas);
      /* if (retorsSelectedGlobal == "moonpalace cancún - nizuc" && date_check_out >= new Date('2020-11-01T00:00:00-05:00') && date_check_out <= new Date('2021-05-30T00:00:00-05:00')) {
        date_check_out = new Date('2020-10-31T00:00:00-05:00');
      }
      if (retorsSelectedGlobal == "moonpalace cancún - nizuc" && date_check_out >= new Date('2021-09-09T00:00:00-05:00') && date_check_out <= new Date('2021-10-24T00:00:00-05:00')) {
        date_check_out = new Date('2021-09-08T00:00:00-05:00');
      } */
      ...
      date_check_out.setDate(date_check_out.getDate() + (dateCheckOut(dater, 7)));
      jQuery('#checkin_date_rf').datepicker('setDatesDisabled', FechasDeshabilitadas);
      jQuery('#checkout_date_rf').datepicker('setDatesDisabled', FechasDeshabilitadas);
      /* if (retorsSelectedGlobal == "moonpalace cancún - nizuc" && date_check_out >= new Date('2020-11-01T00:00:00-05:00') && date_check_out <= new Date('2021-05-30T00:00:00-05:00')) {
        date_check_out = new Date('2020-10-31T00:00:00-05:00');
      } */
      ...
      jQuery('#checkin_date_r_mex_b').datepicker('setDatesDisabled', FechasDeshabilitadas);
      jQuery('#checkout_date_r_mex_b').datepicker('setDatesDisabled', FechasDeshabilitadas);
  ...
  jQuery("section.booking form.form_book_now .icons_booking .ico_resort").click(function () {
    ...
    if (cual == "resortfly") {
      if (jQuery('.booking').attr('data-m') == 'MX') {
        jQuery(this).addClass("active");
        jQuery("section.booking .only_resort.resort_fly").removeClass("onlyresort-mx");
        jQuery("section.booking .only_resort.resort_fly").addClass("resortfly-mx active");

        jQuery("section.booking .only_resort.resort_fly .checkin.checkin-r").hide();
        jQuery("section.booking .only_resort.resort_fly .checkout.checkout-r").hide();

        jQuery("section.booking .only_resort.resort_fly .origin").show();
        jQuery("section.booking .only_resort.resort_fly .easy-autocomplete").show();
        jQuery("section.booking .only_resort.resort_fly .checkin.checkin-rf").show();
        jQuery("section.booking .only_resort.resort_fly .checkout.checkout-rf").show();
        jQuery("section.booking .only_resort.resort_fly li.item-resort").hide();
      ...
      if (jQuery('.booking').attr('data-m') == 'MX') {
        jQuery(this).addClass("active");
        jQuery("section.booking .only_resort.resort_fly").removeClass("resortfly-mx");
        jQuery("section.booking .only_resort.resort_fly").addClass("onlyresort-mx active");

        jQuery("section.booking .only_resort.resort_fly .origin").hide();
        jQuery("section.booking .only_resort.resort_fly .easy-autocomplete").hide();
        jQuery("section.booking .only_resort.resort_fly .checkin.checkin-rf").hide();
        jQuery("section.booking .only_resort.resort_fly .checkout.checkout-rf").hide();

        jQuery("section.booking .only_resort.resort_fly .checkin.checkin-r").show();
        jQuery("section.booking .only_resort.resort_fly .checkout.checkout-r").show();
        jQuery("section.booking .only_resort.resort_fly li.item-resort").show();
  ...
  jQuery("section.booking form.form_book_now .only_resort button#btn_book_now_resort").click(function (e) {
    e.preventDefault();
    if (jQuery('.booking').attr('data-m') == 'MX' && jQuery('.resortfly-mx').length != 0) {
     // console.log("Hotel+vuelo MX ");
      var error = 0;
      var fullurl = '';
      var lang = drupalSettings.booking.language;
      var urldefault = drupalSettings.booking.motor_mexico;
      var origen = jQuery("input#origin").val().substr(0, 3);
      var checkin_date_rf = jQuery("input#checkin_date_rf").val().split('/').join('-');
      var checkout_date_rf = jQuery("input#checkout_date_rf").val().split('/').join('-');
      var totalrooms = parseInt(jQuery(".number_rooms").text());
      var totaladults = []
      var totalchildren = []
      var totalchildrenage = "";
      var tmp = true;
      var validation = "room0.AgeKids";
      ...
      if (window.matchMedia("(max-width: 1200px)").matches) {
        if (cual_resort == undefined) {
          ...
          jQuery(".box_select_resort").css("border-color", "#e4e4e4");
          error = 0
        }

        if (origen != "" && autoc_selecionado == 0) {
          jQuery("section.booking form.form_book_now .resort_fly input#origin").css("border", "1px solid #654628");
          //jQuery("section.booking form.form_book_now .resort_fly .easy-autocomplete").css("border", "1px solid #654628");
        } else {
          error = 1
          jQuery("section.booking form.form_book_now .resort_fly input#origin").css("border", "1px solid #902729");
          // jQuery("section.booking form.form_book_now .resort_fly .easy-autocomplete").css("border", "1px solid #902729");
        }
        if (checkin_date_rf == "") {
          error = 1
          jQuery("input#checkin_date_rf").css("border-color", "#902729");
        } else {
          jQuery("input#checkin_date_rf").css("border-color", "#e4e4e4");
        }
        if (checkout_date_rf == "") {
          error = 1
          jQuery("input#checkout_date_rf").css("border-color", "#902729");
        } else {
          jQuery("input#checkout_date_rf").css("border-color", "#e4e4e4");
        }
      } else {
        if (cual_resort == undefined) {
          jQuery(".box_select_resort").css("border-color", "#902729");
        ...
        if (origen != "" && autoc_selecionado == 0) {
          jQuery("section.booking form.form_book_now .resort_fly input#origin").css("border", "1px solid #4296c3");
          jQuery("section.booking form.form_book_now .resort_fly .easy-autocomplete").css("border", "none");
        } else {
          error = 1
          jQuery("section.booking form.form_book_now .resort_fly input#origin").css("border", "1px solid #902729");
          jQuery("section.booking form.form_book_now .resort_fly .easy-autocomplete").css("border", "none");
      ...
      jQuery(this).parents(".only_resort").find(".main_room_guests").each(function (index) {
        ...
        room = numberRoom(room);
        totaladults.push(room + '.Adults' + '=' + numadult + '&');
      })
      jQuery("section.booking form.form_book_now .only_resort .rooms .main_box_rooms .quantity_rooms .main_room_guests .box_age_children .age_children").each(function (index) {
        ...
        if (numchild != 0) {
          if (tmp) {
            totalchildrenage = "room0.AgeKids=";
            tmp = false;
          }
          totalchildren.push(room + '.Kids' + '=');
          if (validation != room + ".AgeKids") {
            totalchildrenage = cutText(totalchildrenage);
            totalchildrenage += "&" + room + '.AgeKids' + '=';
            validation = room + ".AgeKids";
          }
          totalchildrenage += edadchild + ",";
        }
      });
      var variables = '&Rooms=';
      ...
      totalchildrenage = cutText(totalchildrenage);
      variables += totalchildrenage;

      var textResort = jQuery("section.booking form.form_book_now .only_resort .select_resort .box_select_resort p").text();
      textResort = textResort.trim();
      var idHotel = "";

      switch (textResort) {
        case 'Moon Palace Cancún':
          idHotel = "100409";
          MappingId = "26";
          MappingType = "2";
          urldefault = "https://paquetes.moonpalace.com/paquetes/buscar";
          break;
        case 'Moon Palace Cancun':
          idHotel = "100409";
          MappingId = "26";
          MappingType = "2";
          urldefault = "https://paquetes.moonpalace.com/paquetes/buscar";
          break;
        case 'Moon Palace Jamaica':
          idHotel = "103945";
          MappingId = "13";
          MappingType = "1";
          urldefault = "https://paquetes.moonpalace.com/paquetes/buscar";
          break;
        case 'Moon Palace The Grand - Cancun':
          idHotel = "240611";
          MappingId = "26";
          MappingType = "2";
          urldefault = "https://paquetes.moonpalace.com/paquetes/buscar";
          break;
        case 'The Grand at Moon Palace Cancún':
          idHotel = "240611";
          MappingId = "26";
          MappingType = "2";
          urldefault = "https://paquetes.moonpalace.com/paquetes/buscar";
        ...
        case 'Sun Palace':
          ...
          MappingType = "2";
          break;
        case 'Playacar Palace':
          idHotel = "100469";
          MappingId = "162";
          MappingType = "2";
          break;
        case 'Le Blanc Spa Resort Cancun':
          idHotel = "100348";
          MappingId = "26";
          MappingType = "2";
          urldefault = "https://paquetes.leblancsparesorts.com/paquetes/buscar";
          break;
        case 'Le Blanc Spa Resort Los Cabos':
          idHotel = "311158";
          MappingId = "23";
          MappingType = "1";
          urldefault = "https://paquetes.leblancsparesorts.com/paquetes/buscar";
      ...
      fullurl = urldefault + '?CheckIn=' + formatDat(checkin_date_rf) + '&CheckOut=' + formatDat(checkout_date_rf) + '&CustomAirportCode=' + origen + '&idHotel=' + idHotel + '&MappingId=' + MappingId + '&MappingType=' + MappingType + '&PackageType=0&IsQuote=true&modal=true' + variables;
     ...
     // console.log(fullurl);
      ...
      var ckin_dtr = new Date(jQuery("input#checkin_date_r").val());
      var ckout_dtr = new Date(jQuery("input#checkout_date_r").val());

      var checkin_date_r = ckin_dtr.toISOString().slice(0, 10);
      var checkout_date_r = ckout_dtr.toISOString().slice(0, 10);

      if (!resort_format_date) {
        if (jQuery("section.booking").hasClass('mex') == true) {
          checkin_date_r = changeFormat(jQuery("input#checkin_date_r").val());
          checkout_date_r = changeFormat(jQuery("input#checkout_date_r").val());
        } else {
          ...
          checkin_date_r = [dcheckIn[2], dcheckIn[0], dcheckIn[1]].join('-');
          var dcheckOut = jQuery("input#checkout_date_r").val().split('/');
          checkout_date_r = [dcheckOut[2], dcheckOut[0], dcheckOut[1]].join('-');
        }
      }
      ...
      if (error == 0) {
        ...
        /* Instrucciones solo para la LP de Affiliate */
        if (jQuery('.affiliate.landing-big-deals').length != 0) {
          var textResort = jQuery("section.booking form.form_book_now .only_resort .select_resort .box_select_resort p").text();
          var urlH = "";
          switch (textResort) {
            case 'Beach Palace':
              var urlH = "https://www.reservhotel.com/cancun-mexico/beach-palace-be/booking-engine/ibe5.main?hotel=10397";
              break;
            case 'Cozumel Palace':
              var urlH = "https://www.reservhotel.com/cozumel-mexico/cozumel-palace-be/booking-engine/ibe5.main?hotel=10398";
              break;
            case 'Isla Mujeres Palace':
              var urlH = "https://www.reservhotel.com/isla-mujeres-mexico/isla-mujeres-palace-be/booking-engine/ibe5.main?hotel=10399";
              break;
            case 'Sun Palace':
              var urlH = "https://www.reservhotel.com/cancun-mexico/sun-palace/booking-engine/ibe5.main?hotel=10403";
              break;
            case 'Playacar Palace':
              var urlH = "https://www.reservhotel.com/playa-del-carmen-mexico/playacar-palace/booking-engine/ibe5.main?hotel=10402";
              break;
            case 'Moon Palace The Grand - Cancun':
              var urlH = "https://www.reservhotel.com/cancun-mexico/the-grand-at-moon-palace-cancun/booking-engine/ibe5.main?hotel=10433";
              break;
            case 'Moon Palace Cancun':
              var urlH = "https://www.reservhotel.com/cancun-mexico/moon-palace-cancun-be/booking-engine/ibe5.main?hotel=10381";
              break;
            case 'Moon Palace Cancún - Nizuc':
              var urlH = "https://www.reservhotel.com/cancun-mexico/moon-palace-cancun-nizuc-be/booking-engine/ibe5.main?hotel=10742";
              break;
            case 'Moon Palace Jamaica':
              var urlH = "https://www.reservhotel.com/ocho-rios-jamaica/moon-palace-jamaica-grande-be/booking-engine/ibe5.main?hotel=10401";
              break;
            case 'Le Blanc Spa Resort Cancun':
              var urlH = "https://www.reservhotel.com/cancun-mexico/le-blanc-spa-resort-be/booking-engine/ibe5.main?hotel=10400";
              break;
            case 'Le Blanc Spa Resort Los Cabos':
              var urlH = "https://www.reservhotel.com/los-cabos-mexico/le-blanc-los-cabos-be/booking-engine/ibe5.main?hotel=10458";
              break;
          }
          var aff = valorParametro("aff");

          var aDate = valorParametro("skd-checkin", fullurl);
          aDate = formatDat(aDate);

          var dDate = valorParametro("skd-checkout", fullurl);
          dDate = formatDat(dDate);

          if (aff != null) {
            jQuery.getJSON("https://www.reservhotel.com/win/owa/ta_login_ws.json_get?system_usr=PALACEPRO&agency_id=" + aff, function (data) {
              if (data) {
                if (data.error) {
                  urlH += "&aDate=" + aDate + "&dDate=" + dDate;
                  urlH = urlbooking(urlH);
                 // console.log(urlH);
                  window.open(urlH, "_blank");
                } else {
                  urlH += "&aff=" + aff + "&aDate=" + aDate + "&dDate=" + dDate;
                  urlH = urlbooking(urlH);
                 // console.log(urlH);
                  window.open(urlH, "_blank");
                }
  ...
  /* Función para armar la URL de reserva */
  function urlbooking(urlH) {
    var aDate = "";
    var dDate = "";
    var adults = jQuery('.only_resort .rooms .type_guests_text_adult .box_p .number').text();
    var child = jQuery('.only_resort .rooms .type_guests_text_children .box_p .number').text();
    var rooms = jQuery(".only_resort .rooms .rooms_text_select .box_p .number_rooms").text();
    //urlH += "&aff="+aff+"&aDate="+aDate+"&dDate="+dDate+"&adults="+adults+"&child="+child+"&rooms="+rooms;
    urlH += "&adults=" + adults + "&child=" + child + "&rooms=" + rooms;

    return urlH;
  }

  /* función para formaear una fecha */
  function formatDat(fecha) {
    var userDate = fecha;
  ...
  function valorParametro(nombreParametro, cadena) {
    if (cadena == null) {
      var sPaginaURL = window.location.search.substring(1);
    } else {
      var sPaginaURL = cadena;
    }

    var sURLVariables = sPaginaURL.split('&');
    for (var i = 0; i < sURLVariables.length; i++) {
      var sParametro = sURLVariables[i].split('=');
      if (sParametro[0] == nombreParametro) {
        return sParametro[1];
      }
    }
    return null;
  ...
  /* función que quita el último caracter de una cadena */
  function cutText(chainText) {
    var lengthText = chainText.length;
    return chainText.substring(0, lengthText - 1);
  }

  /* función para solucionar indice de habitaciones */
  function numberRoom(room) {
    switch (room) {
      case "room1":
        room = "room0"
        break;
  ...
  function dateSunpalace(fechas, retorsSelected, moreDays, idPlan) {

    var y = '';
    var m = '';
    var d = '';
    for (var index = FechasDeshabilitadas.length; index > 0; index--) {
      FechasDeshabilitadas.pop();
    }
    var disabledDatesPRB = fechas;
    var res = disabledDatesPRB.split(",");
    for (var i = 0; i < res.length; i++) {
      var validar = res[i].indexOf("-") > -1;
      if (validar == true) {
        var rango = res[i].split("-");
        for (var j = 0; j < rango.length; j++) {
          var fechaInicio = new Date('' + rango[0] + '')
          var fechaFin = new Date('' + rango[1] + '')
        ...
        FechasDeshabilitadas.push(res[i]);
      }
    }
    ...
    if (idPlan == 1) {
      ...
      var date_check_in = new Date();
      date_check_in.setDate(date_check_in.getDate() + getdate);

      var mt = "" + (date_check_in.getMonth() + 1);
      var dy = "" + date_check_in.getDate();
      var ye = "" + date_check_in.getFullYear()
      var fechas = '' + mt + '/' + dy + '/' + ye + '';

      date_check_in.setDate(date_check_in.getDate() + (dateCheckIn(fechas)));
      ...
      var date_check_in_rf = new Date();
      date_check_in_rf.setDate(date_check_in_rf.getDate() + getdate);

      var mt = "" + (date_check_in_rf.getMonth() + 1);
      var dy = "" + date_check_in_rf.getDate();
      var ye = "" + date_check_in_rf.getFullYear()
      var fechas = '' + mt + '/' + dy + '/' + ye + '';

      date_check_in_rf.setDate(date_check_in_rf.getDate() + (dateCheckIn(fechas)));
      ...
      var checkin_date_r_mex_b = new Date();
      checkin_date_r_mex_b.setDate(checkin_date_r_mex_b.getDate() + getdate);

      var mt = "" + (checkin_date_r_mex_b.getMonth() + 1);
      var dy = "" + checkin_date_r_mex_b.getDate();
      var ye = "" + checkin_date_r_mex_b.getFullYear()
      var fechas = '' + mt + '/' + dy + '/' + ye + '';

      checkin_date_r_mex_b.setDate(checkin_date_r_mex_b.getDate() + (dateCheckIn(fechas)));



      jQuery('#checkin_date_r_mex_b').datepicker('setStartDate', new Date());
      jQuery("#checkin_date_r_mex_b").datepicker({ dateFormat: "mm/dd/yy" }).datepicker("setDate", checkin_date_r_mex_b);
  ...
  jQuery("section.booking form.form_book_now .only_resort .select_resort ul li").click(function () {
    ...
    resort_format_date = jQuery(this).data("format-date");
    ...
    var misFechas = jQuery(this).attr('data-down-date');
    /* Número de días a partir hoy para calcular la fecha por defecto. */
    if (jQuery(this).attr('get-date') == "") {
      getdate = 15;
    } else {
      getdate = parseInt(jQuery(this).attr('get-date'));
    }
    dateSunpalace(misFechas, retorsSelected, 15, 1);
  ...
  jQuery("section.booking form.form_book_now .resort_fly .box_inputs .select_resort_f ul li").click(function () {
    ...
    dateSunpalace(misFechas, retorsSelected, 17, 2);



  })

  //campo fecha de sección discoveries
  jQuery('form .checkin-discoveries-en').datepicker({
    autoclose: true,
    format: 'yyyy-mm-dd',
    startDate: setDate5,
    disableTouchKeyboard: true,
  ...
  jQuery('form .checkin-discoveries-es').datepicker({
    autoclose: true,
    format: 'yyyy-mm-dd',
    startDate: setDate5,
    disableTouchKeyboard: true,
  ...
  jQuery(".landing-2-1 select.booking-hotel").change(function () {
    ...
    if (retorsSelected2x1 == "le blanc spa resort cancun" || retorsSelected2x1 == "le blanc spa resort los cabos" || retorsSelected2x1 == "the grand at moon palace cancun" || retorsSelected2x1 == "sun palace") {
      ...
      if (language_site == 'en') {
        ...
        jQuery('.date-check .checkin').datepicker('setDate', setDate);
  ...
  jQuery("section.booking.mex form.form_book_now .resort_fly .sr_mex ul li").click(function () {
    ...
    dateSunpalace(misFechas, retorsSelected, 17, 3);
...
function booking_scroll() {
  ...
  jQuery(window).scroll(function () {
    if (jQuery(this).scrollTop() > alturabooking) {
      ...
      jQuery(".landing-2-1 section.booking").removeClass("min");
      jQuery("section.booking").css("top", 80);
...
/* navbar rel */
function navbar_rel_target() {
  jQuery("button.owl-dot").attr("aria-label", "button");
...
function acordeon_intervaled_module() {
  var botonLeer = jQuery("section.palace-resort-interleaved-module.type-actividad .content article .copy .box_copys .redmore");
  botonLeer.each(function () {
    var altoContenido = jQuery(this).parent().find(".box-detalle").height();
   // console.log(altoContenido)
    if (altoContenido < 200) {
      jQuery(this).hide();
...
function functionslp2x1() {
  ...
  if (language_site == 'es') {
    ...
    jQuery('.date-check .form-control').datepicker('setDate', setDate2);
  }
...
//Variables globales para los timers y contenido en general
var placeId = 0;
var timeoutId = 0;
var timeoutId2 = 0;
var timeoutIdExp = 0;
var timeoutIdExp2 = 0;
var timeoutIdAmenitie = 0;
var subtitle = '';
var benefits = [];
var amenities = [];
var mainBannerHotel = [];
...
//Funcion para crear el tab de diferentes restaurantes, aventuras, cuartos
function detailExperienceSlider(items) {


  if (jQuery(window).width() > 768) {

    if (items >= 5) {
      jQuery('.item__container.owl-carousel').owlCarousel({
        items: 5,
        loop: true,
        margin: 0,
        center: true,
  ...
  if (jQuery(window).width() < 768) {
    jQuery('section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container').css("display", "block")
...
//Funcion para crear el tab de diferentes restaurantes, aventuras, cuartos
function detailAmenitiesSlider(items) {

  if (jQuery(window).width() > 768) {
    if (items >= 5) {
      jQuery('.tabs-int.owl-carousel').owlCarousel({
        items: 5,
        loop: true,
        margin: 0,
        nav: true,
  ...
  if (jQuery(window).width() < 768) {
    if (items >= 2) {
      jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').css("display", "block")
...
//Funcion para crear en mobile el carousel de los destinos
function destineMobileSlider() {
  jQuery('.slider-tabs-destinos-mobile.owl-carousel').owlCarousel({
    items: 1,
    loop: true,
    margin: 0,
    center: true,
  ...
  jQuery('.slider-tabs-destinos-mobile.owl-carousel').on('click', '.owl-item.center li', function (e) {
    placeId = jQuery(this).attr("dataid");
    subtitle = jQuery(this).attr("datasubtitle");
    dataDestine['id'] = placeId;
    dataDestine['subtitle'] = subtitle;
    dataDestine['first'] = true
    jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
    jQuery('.info-tabs.owl-carousel').owlCarousel('destroy');

    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').text('');
    jQuery('.tabs-int.owl-carousel').owlCarousel('destroy');
    jQuery('.slider-exclusive.owl-carousel').owlCarousel('destroy');
    jQuery('section.main-content-ads .slider-exclusive').text('');
    jQuery('.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .info-tabs').text('');

    jQuery("section.main-our-resorts .wrapper .main-section-hotel .brand-hotel").text("")
    jQuery("section.main-our-resorts .wrapper .main-section-hotel h3").text("")
    jQuery("section.main-our-resorts .wrapper .main-section-hotel .hotel__description").text("")

    jQuery('.main-slider-hotel.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-slider-hotel').text('');
    jQuery('.tabs-brand-hotels.owl-carousel').owlCarousel('destroy');
    jQuery('.item__container.owl-carousel').owlCarousel('destroy');
    jQuery("section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container").text('')
    jQuery('.main-our-resorts .wrapper .container-brand-hotel .tabs-brand-hotels').text('')
    clearTimeout(timeoutId);
    clearTimeout(timeoutId2);
    clearTimeout(timeoutIdExp);
    clearTimeout(timeoutIdExp2);
    jQuery('.main-slider-hotel.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-slider-hotel').text('')
    // jQuery('.slider-tabs-destinos-mobile').owlCarousel('destroy');
    jQuery('.slider-benefits-mobile.owl-carousel').owlCarousel('destroy');
    jQuery('.tabs-main-amenities-mobile.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .benefits .slider-benefits-mobile').text('');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities-mobile').text('');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie').text('');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #descAmenitie').text('');
    jQuery('.tabs-main-amenities').text('');
    jQuery('.benefits-desktop').text('');


    sessionStorage.setItem("dataDestine", JSON.stringify(dataDestine));
    if (sessionStorage.getItem('homePath') == window.location.pathname) {

      var scroll = jQuery("section.main-our-resorts").offset().top - 100
      jQuery("html, body").animate({ scrollTop: scroll }, "slow");
    }
    // console.log("Id hotel:", placeId)
    //console.log("Subitle:", subtitle)
    viewInfoDestine(placeId, subtitle);
  });
}

//Funcion para renderizar el slider de beneficios
function sliderBenefits() {
  jQuery('.slider-benefits-mobile.owl-carousel').owlCarousel({
    items: 1,
    loop: true,
    margin: 0,
    center: true,
...
//Funcion para renderizar el slider de amenidades
function sliderAmenities() {
  jQuery('.tabs-main-amenities-mobile.owl-carousel').owlCarousel({
    items: 1,
    loop: true,
    margin: 0,
    center: true,
...
//Funcion para slider del initial modal
function mainSlider() {
  jQuery('.slider-items.owl-carousel').owlCarousel({
    items: 1,
    loop: true,
    margin: 0,
    center: true,
  ...
  //Funcion para visualizar la informacion del destino clickeado
  jQuery('.slider-items.owl-carousel').on('click', '.owl-item.center .item .wrap .cta .btn-white ', function (e) {
    placeId = jQuery(this).attr("dataid");
    subtitle = jQuery(this).attr("datasubtitle");
    dataDestine['id'] = placeId;
    dataDestine['subtitle'] = subtitle;
...
//Funcion para el carousel de hoteles
function sliderHotel(data, subtitle, items) {
  // console.log("cantidad items", items)


  if (jQuery(window).width() > 768) {
    if (items >= 5) {
      jQuery('.main-our-resorts .wrapper .tabs-brand-hotels.owl-carousel').owlCarousel({
        items: 5,
        loop: true,
        margin: 0,
        nav: true,
  ...
  if (jQuery(window).width() < 768) {
    ...
    jQuery('section.main-our-resorts .wrapper .container-brand-hotel .tabs-brand-hotels').css("display", "block")
  ...
  //console.log("Hoteles del destino: ", data);
  var hotelsIds = [];
  //Variable para guardar los IDs de los beneficios y amenidades
  var arrayBenefitsIds = [];
  var arrayAmenetiesIds = [];
  //Variable para guardar el response del servicio
  var dataBenefits = [];
  var dataAmeneties = [];


  //Valores por defecto en la vista
  var title = data[0].title[0].value;
  var description = data[0].field_descripcion_[0].processed;
  var logo = data[0].field_logo[0].url;
  var titleBenefits = data[0].field_titulo_beneficio[0].value
  var titleAmenities = data[0].field_titulo_amenidad[0].value
  amenities = data[0].field_amenidades;
  benefits = data[0].field_lista_beneficios;
  mainBannerHotel = data[0].field_image
  colorHotel = data[0].field_color_propiedad[0].value;
  urlBooking = data[0].field_book_now_flight[0].uri
  idBookingHotelBtn = data[0].field_id_button_booking_resort[0].value
  titleBooking = data[0].field_book_now_flight[0].title
  dataHotel['title'] = title
  dataHotel['description'] = description
  dataHotel['logo'] = logo
  dataHotel['titleBenefits'] = titleBenefits
  dataHotel['titleAmenities'] = titleAmenities
  dataHotel['ameneties'] = amenities
  dataHotel['benefits'] = benefits
  dataHotel['mainBannerHotel'] = mainBannerHotel
  dataHotel['urlbookin'] = urlBooking
  dataHotel['bookinTitle'] = titleBooking
  sessionStorage.setItem('dataHotel', JSON.stringify(dataHotel))

  //console.log("Title", data[0].field_titulo_beneficio[0])

  jQuery.each(amenities, function (index, data) {
    // console.log("Ids Amenidades: ", data.target_id)
    var amenitiesIds = data.target_id;
    arrayAmenetiesIds.push(amenitiesIds);
  });
  jQuery.each(benefits, function (index, data) {
    //console.log("Beneficios: ", data.target_id)
    var benefitsIds = data.target_id;
    arrayBenefitsIds.push(benefitsIds);
  })
  //Funcion para renderizar los slider del banner principal del hotel
  jQuery.each(mainBannerHotel, function (index, data) {
    // console.log("Src banner main", data)
    var srcBannerImg = data.url
    jQuery('.main-our-resorts .wrapper .main-slider-hotel').append(
      "<div class = 'item'>" + "<picture>" +
      "<img class='lazyloaded' src='" + srcBannerImg + "' alt='banner-hotel'>" + "</picture>" +
      "</div>"
    )
  })
  sliderHotelBanner()
  viewAmenities(arrayAmenetiesIds, dataAmeneties);
  viewBenefits(arrayBenefitsIds, dataBenefits);

  //Funciones para renderizar contenido
  jQuery("section.main-our-resorts .wrapper .main-section-hotel h3").text(title);
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .benefits #title-benefit").text(titleBenefits);
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities #title-amenitie").text(titleAmenities);
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .hotel__description").html(description);
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .brand-hotel img").attr("src", logo);
  jQuery("section.main-our-resorts .wrapper h4.title-tabs-hotels").text(subtitle)
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .content-btn-book .btn-brown").attr("href", urlBooking)
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .content-btn-book .btn-brown").attr("id", idBookingHotelBtn)
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .content-btn-book .btn-brown").text(titleBooking)
  jQuery("section.main-our-resorts .wrapper .main-section-hotel .content-btn-book .btn-brown").css("background-color", colorHotel)



  //Funcion para obtener el ID del hotel clickeado en el carousel
  if (items >= 5) {
    jQuery('.tabs-brand-hotels.owl-carousel').on('click', '.owl-item .item', function (e) {
      clearTimeout(timeoutId);
      clearTimeout(timeoutId2);
      // clearTimeout(timeoutIdExp);
      // clearTimeout(timeoutIdExp2);
      ...
      //Limpio los arrays para los nuevos datos
      dataBenefits = [];
      dataAmeneties = [];
      arrayBenefitsIds = [];
      arrayAmenetiesIds = [];

      //Obtendo el Id del hotel clickeado
      var hotelId = jQuery(this).attr("datahotel");
      //console.log("Id hotel:", hotelId);

      //Funcion para recorrer la data del destino y buscar el id del hotel para obtener su infomacion
      jQuery.each(data, function (index, data) {
        //console.log(index, data.nid[0].value);
        var nids = data.nid[0].value;
        hotelsIds.push(nids);

      });
      //Obtengo el index del dato hotel que se le da click
      var indexHotel = hotelsIds.findIndex(function (nid) {
        if (nid == hotelId) {
          return nid;
        }
      });

      amenities = data[indexHotel].field_amenidades;
      benefits = data[indexHotel].field_lista_beneficios;
      mainBannerHotel = data[indexHotel].field_image;
      urlBooking = data[indexHotel].field_book_now_flight[0].uri;
      idBookingHotelBtn = data[indexHotel].field_id_button_booking_resort[0].value
      titleBooking = data[indexHotel].field_book_now_flight[0].title;
      colorHotel = data[indexHotel].field_color_propiedad[0].value;
      ...
      //console.log()
      jQuery.each(amenities, function (index, data) {
        console.log("Amenidades: ", data.target_id)
        var amenitiesIds = data.target_id;
        arrayAmenetiesIds.push(amenitiesIds);
      });
      jQuery.each(benefits, function (index, data) {
      ...
      //console.log(indexHotel);
      var titleHotel = data[indexHotel].title[0].value;
      var descriptionHotel = data[indexHotel].field_descripcion_[0].processed;
      var logoHotel = data[indexHotel].field_logo[0].url;
      var titleBenefits = data[0].field_titulo_beneficio.value
      var titleAmenities = data[0].field_titulo_amenidad.value

      dataHotel['title'] = titleHotel
      dataHotel['description'] = descriptionHotel
      dataHotel['logo'] = logoHotel
      dataHotel['titleBenefits'] = titleBenefits
      dataHotel['titleAmenities'] = titleAmenities
      dataHotel['ameneties'] = amenities
      dataHotel['benefits'] = benefits
      ...
      //Funciones para renderizar contenido
      jQuery("section.main-our-resorts .wrapper .main-section-hotel h3").text(titleHotel);
      jQuery("section.main-our-resorts .wrapper .main-section-hotel .hotel__description").html(descriptionHotel);
      jQuery("section.main-our-resorts .wrapper .main-section-hotel .brand-hotel img").attr("src", logoHotel);
      jQuery("section.main-our-resorts .wrapper .main-section-hotel .benefits #title-benefit").text(titleBenefits);
      jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities #title-amenitie").text(titleAmenities);
    });
  } else {

    jQuery("section.main-our-resorts .wrapper .container-brand-hotel .tabs-brand-hotels .item").click(function () {
      clearTimeout(timeoutId);
      clearTimeout(timeoutId2);
      // clearTimeout(timeoutIdExp);
      // clearTimeout(timeoutIdExp2);
...
//Funcion para crear los contenidos del tab brands, la lista desplegable en navbar y el slider de hoteles en mobil
function destinationSelect(data) {
  var item = data.length
  jQuery("header #block-headermenu ul .first .navbar-text").append(
    "<ul class = 'destinations--select' >" + "</ul>"
  )
  jQuery(data).each(function (i, value) {
    var nameDestine = value.name[0].value;
    var IDModal = value.tid[0].value
    var subtitleDestine = value.field_subtitulo[0].value
    //console.log(nameDestine)
    jQuery("header #block-headermenu ul li .navbar-text .destinations--select").append(
      "<li dataid=" + IDModal + " datasubtitle='" + subtitleDestine + "' >" + nameDestine + "</li>"
    )
    jQuery("section.main-our-resorts .wrapper .tabs-destinos ul").append(
      "<li dataid=" + IDModal + " datasubtitle='" + subtitleDestine + "' >" + nameDestine + "</li>"
    )
    jQuery("section.main-our-resorts .wrapper .slider-tabs-destinos-mobile-container .slider-tabs-destinos-mobile").append(
      "<li dataid=" + IDModal + " datasubtitle='" + subtitleDestine + "' >" + nameDestine + "</li>"
    )
  })
  destineMobileSlider(item);


  jQuery('section.main-our-resorts .wrapper .tabs-destinos ul li').on('click', function () {
    jQuery(" section.main-our-resorts .wrapper .tabs-destinos ul > li.active").removeClass("active");
    jQuery(this).addClass("active");
  });



  jQuery("header #block-headermenu ul li .navbar-text .destinations--select li").click(function (e) {
    jQuery('.info-tabs.owl-carousel').owlCarousel('destroy');
    jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').text('');
    ...
    var subtitle = jQuery(this).attr("datasubtitle")
    ...
    viewInfoDestine(id, subtitle);
  })
  jQuery("section.main-our-resorts .wrapper .tabs-destinos ul li").click(function (e) {
    jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
    jQuery('.info-tabs.owl-carousel').owlCarousel('destroy');
...
function Modal() {
  sessionStorage.setItem("homePath", window.location.pathname)
  //Consulta servicio modal
  jQuery.ajax({
    type: 'get',
    url: '/json/destinos',
  }).done(function (data) {
    //console.log(data)
    sessionStorage.setItem("allDataDestine", JSON.stringify(data))
    jQuery(data).each(function (i, value) {
      var nameModal = value.name[0].value
      var IDModal = value.tid[0].value
      var subtitleDestine = value.field_subtitulo[0].value
      var imgModal = value.field_imagen_home[0].url
      var altModal = value.field_imagen_home[0].alt
      var titleModal = value.field_imagen_home[0].title
      var descriotionModal = value.description[0].value

      // var titleBooking = value.field_book_now_flight[0].title


      jQuery('.initial-modal .wrapper .slider-items').append("<div class='item'>" +
        "<div class='wrap'>" +
        "<span class='ico'></span>" +
        "<h3>" + nameModal + "</h3>" +
        "<div class='cta'>" +
        descriotionModal +
        "<div class='btn-white' dataid='" + IDModal + "' datasubtitle='" + subtitleDestine + "'  >VIEW MORE</div>" +
        "</div>" +
        "</div>" +
        " <picture>" +
        " <img src='" + imgModal + "' title='" + titleModal + "' alt='" + altModal + "' loading='lazy'>" +
        "</picture>" +
        "</div>");

      //Funcion para crear elementos en vista mobile
      jQuery('.initial-modal .wrapper .view-mobile').append("<div class='item'>" +
        "<div class='wrap'>" +
        "<span class='ico'></span>" +
        "<h3>" + nameModal + "</h3>" +
        "<div class='cta'>" +
    ...
    ///Vista mobile boton banner inicial
    jQuery(".initial-modal .wrapper .view-mobile .item .wrap .cta .btn-white").click(function (e) {
      var destinoId = jQuery(this).attr("dataid");
      var subtitle = jQuery(this).attr("datasubtitle");
      //console.log(hotelId)
      dataDestine['id'] = destinoId;
      dataDestine['subtitle'] = subtitle;
      dataDestine['first'] = true;

      sessionStorage.setItem("dataDestine", JSON.stringify(dataDestine));
      viewInfoDestine(destinoId, subtitle)
    });

    //Funcion para elegir un destino al azar y cerrar el modal inicial
    jQuery(".initial-modal .close").click(function () {
      var cantidadDestinos = data.length
      var id = Math.floor((Math.random() * cantidadDestinos) + 0)
      console.log("aleatorio: ", data[id].tid[0].value)
      console.log("data aleatoria: ", data[id].field_subtitulo[0].value)
      //Modal Slider
      var idHotel = data[id].tid[0].value
      var subtitleHotel = data[id].field_subtitulo[0].value

      dataDestine['id'] = idHotel;
      dataDestine['subtitle'] = subtitleHotel;
      dataDestine['first'] = true;

      sessionStorage.setItem("dataDestine", JSON.stringify(dataDestine));
      viewInfoDestine(idHotel, subtitleHotel);
      jQuery(".initial-modal").fadeOut(300);
    })

    //Modal Slider
    mainSlider();

    destinationSelect(data);
  })
}


///Funcion para ver data del destino
function viewInfoDestine(id, subtitle) {
  //Obtener hoteles del destino
  var items = 0
  var dataExperience = [];
  var dataADS = [];
  if (id != 0) {
    ...
    //Funcion para solicitar los hoteles del destino
    jQuery.ajax({
      type: 'get',
      url: '/json/hoteles/ ' + id + '',
    }).done(function (allDataDestine) {
      //console.log("Informacion Hotel : ", data);

      jQuery(allDataDestine).each(function (i, dataHotel) {
        var logoBannerHotel = dataHotel.field_logo[0].url;
        var idBannerHotel = dataHotel.nid[0].value;
        colorHotel = dataHotel.field_color_propiedad[0].value;
        idBookingHotelBtn = dataHotel.field_id_button_booking_resort[0].value
        var altBannerHotel = dataHotel.field_logo[0].alt;
        //urlBooking = dataHotel.field_book_now_flight[0].uri
        titleBooking = dataHotel.field_book_now_flight[0].title
        jQuery(".initial-modal").fadeOut(300);

        //Funcion para renderizar los tabs de hoteles
        jQuery('.main-our-resorts .wrapper .container-brand-hotel .tabs-brand-hotels').append(
          "<div class = 'item' dataColorHotel= '" + colorHotel + "' dataHotel= '" + idBannerHotel + "' databooking='" + urlBooking + "' databookingtitle='" + titleBooking + "' >" +
          "<img class='lazyloaded' src='" + logoBannerHotel + "' alt='" + altBannerHotel + "'>" +
          "</div>"
        )
      })
      items = allDataDestine.length
      //Funcion para renderizar los hoteles de cada destino en el carousel
      sliderHotel(allDataDestine, subtitle, items);

    })

    //Funcion para solicitar la informacion del destino
    jQuery.ajax({
      type: 'get',
      url: '/json/informacion_destinos/ ' + id + '',
    }).done(function (infoDestine) {
      //console.log("Informacion Destino : ", infoDestine);
      jQuery("section.main-our-resorts .wrapper .main-content-experiences .title #title-experience").text(infoDestine[0].field_titulo_[0].value)
      jQuery("section.main-our-resorts .wrapper .main-content-experiences .title #description-experience").html(infoDestine[0].field_descripcion_corta[0].processed)
      jQuery("#specialOffers .wrapper .title #exclusive-title").text(infoDestine[0].field_titulo[0].value)
      jQuery.each(infoDestine[0].field_experiencia, function (index, exprecience) {
        jQuery.ajax({
          type: 'get',
          url: '/json/benefits/' + exprecience.target_id + '',

        }).done(function (data) {

          //console.log("Datas experience: ", data);
          dataExperience.push(data[0]);
        })
      });
      jQuery.each(infoDestine[0].field_ofertas_destino, function (index, oferta) {
        jQuery.ajax({
          type: 'get',
          url: '/json/benefits/' + oferta.target_id + '',

        }).done(function (data) {

          // console.log("Datas oferta: ", data);
          dataADS.push(data[0]);
        })
      });

      timeoutIdExp = setTimeout(function () {
        detailsExperience(dataExperience)
        detailADS(dataADS)
        // console.log("Datos Experiencia: ", dataAmeneties)
      }, 3000);
...
function detailADS(dataADS) {
  console.log("Data ADS", dataADS)
  var items = 0;
  items = dataADS.length

  jQuery(dataADS).each(function (i, ads) {

    var srcADS = ads.field_icono[0].url
    var titleADS = ads.field_titulo_color_1[0].value;
    var descADS = ads.field_descripcion[0].processed
    var descuentoADS = ads.field_titulo_banner[0].value
    var porcentajeADS = ads.field_titulo_color_2[0].value
    var offADS = ads.field_subtitulo_banner[0].value
    console.log("descripcion", descADS)
    jQuery("section.main-content-ads .slider-exclusive").append(
      "<div class = 'items'>" +
      "<div class = 'img'>" +
      "<img class = 'lazyloaded' src='" + srcADS + "' alt='ads palace'>" +
      "</div>" +
      "<div class = 'info'>" +
      "<h3>" + titleADS + "</h3>" +
      "<div>" + descADS + "</div>" +
      "<div class = 'discount'>" +
      "<span class='number'> " + descuentoADS + "</span>" +
      "<div class = 'wrap'>" +
      "<span class = 'percentage'>" + porcentajeADS + "</span>" +
      "<span class = 'copy'>" + offADS + "</span>" +
      "</div>" +
      "</div>" +
      "<a class = 'btn-blue' id = '" + idBookingHotelBtn + "' href= '" + urlBooking + "'>" + 'BOOK NOW' + "</a>" +
      "</div>" +
      "</div>"
    )
  })
...
//Funcion para ver los datlles de la experiencia
function detailsExperience(dataExperience) {
  console.log("Data Experience", dataExperience)
  var experienceIds = [];
  //Variable para crear un array de ids de experiencia
  var targetIdExperience = [];
  //Variable para guardar el response del server de las consultas
  var dataIdsExperiences = [];
  //Variable para crear el array de la data necesaria para renderizar
  var sliderDataExperience = []

  //Dejo por defecto la primera experiencia para renderizar contenido
  targetIdExperience = dataExperience[0].field_experiencia;

  var items = 0;
  items = dataExperience.length;
  //Funcion para renderizar los iconos de las experiencias en el slider
  jQuery(dataExperience).each(function (i, experience) {

    var srcExperience = experience.field_icono[0].url
    var descExperience = experience.field_label_cta[0].value;
    var idExperience = experience.id[0].value
    console.log("Experiene id", idExperience)
    if (i == 0) {
      jQuery("section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container").append(
        "<div class = 'item active' idExperience= '" + idExperience + "'>" + "<span class = 'ico'>" +
        "<img class='lazyloaded' src='" + srcExperience + "' alt='icon-benefits'>" +
        "</span>" + "<p>" + descExperience + "</p>" + "</div>"
      )
    } else {
      jQuery("section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container").append(
        "<div class = 'item' idExperience= '" + idExperience + "'>" + "<span class = 'ico'>" +
        "<img class='lazyloaded' src='" + srcExperience + "' alt='icon-benefits'>" +
        "</span>" + "<p>" + descExperience + "</p>" + "</div>"
      )
    }
  ...
  if (items >= 5) {
    jQuery('.item__container.owl-carousel').on('click', '.owl-item .item', function (e) {

      //Funcion para poner el estado activo
      jQuery(function () {
        jQuery('section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container .item').on('click', function () {
      ...
      sliderDataExperience = [];
      jQuery('.info-tabs.owl-carousel').owlCarousel('destroy');
      jQuery('.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .info-tabs').text('');
      var id = jQuery(this).attr("idExperience");
      //console.log("Id experiencia", id)

      jQuery.each(dataExperience, function (index, data) {
        //console.log(index, data.nid[0].value);
        var nids = data.id[0].value;
        experienceIds.push(nids);

      });

      var indexHotel = experienceIds.findIndex(function (nid) {
        if (nid == id) {
          return nid;
        }
      });
      targetIdExperience = dataExperience[indexHotel].field_experiencia;
      // console.log("Experiencia seleccionada: ", targetIdExperience);

      //Funcion para solicitar los detalles de la experiencia
      jQuery.each(targetIdExperience, function (index, id) {
        jQuery.ajax({
          type: 'get',
          url: '/json/benefits/' + id.target_id + '',

        }).done(function (data) {
          console.log("Experience:", data);
          sliderDataExperience.push(data[0])
          // console.log("Datas a: ", sliderDataExperience);
        })
      });

      timeoutIdExp2 = setTimeout(function () {
        viewExperiences(sliderDataExperience)
        //console.log("Datos Experiences Slider: ", sliderDataExperience)
      }, 3000);
    })

  } else {
    jQuery("section.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .item__container .item").click(function () {
      ...
      ///Inicio para renderizar las experiencias
      sliderDataExperience = [];
      jQuery('.info-tabs.owl-carousel').owlCarousel('destroy');
      jQuery('.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .info-tabs').text('');
      var id = jQuery(this).attr("idExperience");
    ...
    // detailExperienceSlider(items)

  }


  //Llamo al slider de experiencias
  detailExperienceSlider(items)

  //Funcion para consultar el contenido de la experiencia para renderizar en el slider
  jQuery.each(targetIdExperience, function (index, id) {
    //console.log("ID de las experiencias: ", id.target_id)
    dataIdsExperiences.push(id.target_id)

  })

  //Funcion para solicitar los detalles de la experiencia
  jQuery.each(dataIdsExperiences, function (index, id) {
    jQuery.ajax({
      type: 'get',
      url: '/json/benefits/' + id + '',

    }).done(function (data) {
      //console.log("Experience:", data);
      sliderDataExperience.push(data[0])
      //console.log("Datas a: ", dataAmeneties);
    })
  });

  timeoutIdExp2 = setTimeout(function () {
    viewExperiences(sliderDataExperience)
    // console.log("Datos Experiences: ", dataAmeneties)
  }, 3000);
}


//Funcion para contruir la data de experiencias del destino
function viewExperiences(sliderDataExperience) {
  //console.log("Funcion Slider", sliderDataExperience)
  var items = sliderDataExperience.length
  jQuery(sliderDataExperience).each(function (i, data) {
    //console.log("Slider", data.field_imagen_desktop[0])
    // //Funcion para renderizar en desktop
    var srcSliderExperience = data.field_imagen_desktop[0].url;
    var subtutleSliderExperience = data.field_descripcion[0].value;
    var titleSliderExperience = data.field_titulo_banner[0].value;
    jQuery('.main-our-resorts .wrapper .main-content-experiences .tabs-experiences .info-tabs').append(
      "<div class = 'item-tab'>" +
      "<div class = 'img'>" +
      "<img class= 'lazyloaded' src='" + srcSliderExperience + "' alt='icon-benefits'>" +
      "</div>" +
      "<div class = 'info'>" +
      "<h3>" + titleSliderExperience + "</h3>" +
      subtutleSliderExperience +
      "</div>" +
      "</div>"
    );
  });
  sliderExperiencesDestine(items)
}

//Funcion para construir la data de amenidades del hotel
function viewAmenities(ids, dataAmeneties) {
  // console.log("Ids amenidades:", ids)
  ...
  jQuery.each(ids, function (index, id) {
    jQuery.ajax({
      ...
      dataAmeneties.push(data[0]);
      //console.log("Datas SubItem Amenidad: ", data[0].field_amenidad);
    })
  });

  timeoutId = setTimeout(function () {
    detailsAmenities(dataAmeneties)
    //console.log("Datos Amenidades: ", dataAmeneties)
  }, 3000);

}

//Funcion para construir la data de beneficios del hotel
function viewBenefits(ids, dataBenefits) {
  //console.log("Ids beneficios:", ids)
  jQuery.each(ids, function (index, id) {
    jQuery.ajax({
      type: 'get',
      url: '/json/benefits/' + id + '',
      ...
      dataBenefits.push(data[0]);
      // console.log("Datas b: ", dataBenefits);
    })
  });
  timeoutId2 = setTimeout(function () {
    //console.log("Datos beneficios: ", dataBenefits);
    detailsBenefits(dataBenefits)
  }, 3000);


}

//Funcion para renderizar los beneficios del hotel
function detailsBenefits(data) {
  // var iconBenefits = '';
  var descBenefits = '';
  ///Recorro el array dataBenefits
  jQuery(data).each(function (i, dataBeneficio) {
    //console.log("data beneficio", dataBeneficio)    //console.log("Detalle Beneficios: ", dataBeneficio.field_icono[0].url, dataBeneficio.field_label_cta[0].value);
    // console.log("icono be", dataBeneficio.field_icono)

    // if (dataBeneficio.field_icono) {
    //   iconBenefits = dataBeneficio.field_icono[0].url;
    // } else {
    //   iconBenefits = ''
    // }
    if (dataBeneficio.field_label_cta) {
      descBenefits = dataBeneficio.field_label_cta[0].value;
    } else {
      descBenefits = ''
    }

    // //Funcion para renderizar en desktop
    jQuery('.main-our-resorts .wrapper .main-section-hotel .benefits .benefits-desktop').append(
      "<li>" + "<span>" +
      //"<img class='lazyloaded' src='" + iconBenefits + "' alt='icon-benefits'>" +
      "</span>" + descBenefits + "</li>"
    );

    //Funcion para renderizar carousel en mobile
    jQuery('.main-our-resorts .wrapper .main-section-hotel .benefits .slider-benefits-mobile').append(
      "<li>" + "<span>" +
      //"<img class='lazyloaded' src='" + iconBenefits + "' alt='icon-benefits'>" +
      "</span>" + descBenefits + "</li>"
    );
  ...
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .benefits .benefits-desktop > li').append("<style>section.main-our-resorts .wrapper .main-section-hotel .benefits .benefits-desktop > li::before{color: " + colorHotel + " !important;}</style>");
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item').append("<style>.tabs-main-amenities > .item.active::before{background-color: " + colorHotel + " !important;}</style>");



  sliderBenefits()

  jQuery('section.main-our-resorts .wrapper .main-section-hotel .benefits .slider-benefits-mobile .owl-dots .owl-dot').append("<style>section.main-our-resorts .wrapper .main-section-hotel .benefits .slider-benefits-mobile .owl-dots .owl-dot.active::after{border: 1px solid " + colorHotel + " !important;}</style>");

  jQuery('section.main-our-resorts .wrapper .main-section-hotel .benefits .slider-benefits-mobile .owl-dots .owl-dot').css("background-color", colorHotel)


}


//Funcion para renderizar las amenidades del hotel
function detailsAmenities(data) {
  console.log("Details Amenitie", data)
  var iconAmenitie = '';
  var descAmenitie = '';
  var idAmenitie = 0;
  var dataAmenitieRender = []
  var newOrdenData = ['Rooms', 'All Inclusive', 'Restaurants', 'Spa', 'Golf', 'Activities', 'Entertainment'];
  //dataAmenitieRender.push(data[i].field_label_cta[0].value)
  var i = 0;
  for (i = 0; i < newOrdenData.length; i++) {
    var indice = data.findIndex(function (amenidad, indice) {
      if (amenidad.field_label_cta[0].value == newOrdenData[i]) {
        return true;
      }
    });
    if (indice >= 0) {
      dataAmenitieRender.push(data[indice])

    }
  }
  console.log("data organizada", dataAmenitieRender)

  jQuery(dataAmenitieRender).each(function (i, dataAmenidad) {


    var target_id = dataAmenidad.id[0].value
    if (dataAmenidad.field_icono) {
      iconAmenitie = dataAmenidad.field_icono[0].url;
    } else {
      iconAmenitie = ''

    }
    if (dataAmenidad.field_label_cta) {
      descAmenitie = dataAmenidad.field_label_cta[0].value;
    } else {
      descAmenitie = ''
    }
    ...
    if (i == 0) {
      // //Funcion para renderizar en desktop
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities').append(
        "<div id= 'amenidad" + i + "' class = 'item active' dataAmenitie= '" + target_id + "'>" + "<span class = 'ico'>" +
        "<img class='lazyloaded' src='" + iconAmenitie + "' alt='icon-benefits'>" +
        "</span>" + "<p>" + descAmenitie + "</p>" + "</div>"
      );

      //Funcion para renderizar carousel en mobile
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities-mobile').append(
        "<div id= 'amenidad" + i + "' class = 'item active' dataAmenitie= '" + target_id + "' >" + "<span class = 'ico'>" +
        "<img class='lazyloaded' src='" + iconAmenitie + "' alt='icon-benefits'>" +
        "</span>" + "<p>" + descAmenitie + "</p>" + "</div>"
      );
      ...
      // //Funcion para renderizar en desktop
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities').append(
        "<div id= 'amenidad" + i + "' class = 'item' dataAmenitie= '" + target_id + "'>" + "<span class = 'ico'>" +
        "<img class='lazyloaded' src='" + iconAmenitie + "' alt='icon-benefits'>" +
        "</span>" + "<p>" + descAmenitie + "</p>" + "</div>"
      );
  ...
  if (dataAmenitieRender[0]) {

    idAmenitie = dataAmenitieRender[0].id[0].value;
  }
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item').append("<style>.tabs-main-amenities > .item.active::before{background-color: " + colorHotel + " !important;}</style>");

  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item.active > p').css("color", colorHotel)

  //Funcion paa saber el id de la amenidad y realizar una nueva consulta
  jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item").click(function (e) {
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item > p').css("color", "black")
    jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie').text('');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #descAmenitie').text('');
    clearTimeout(timeoutIdAmenitie);

    jQuery('.tabs-int.owl-carousel').owlCarousel('destroy');
    jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').text('');
    var targetId = jQuery(this).attr("dataAmenitie");
    //console.log(targetId);
    amenitieContent(targetId)
  });
  //Funcion paa saber el id de la amenidad y realizar una nueva consulta slider mobile
  jQuery('.tabs-main-amenities-mobile.owl-carousel').on('click', '.owl-item .item', function (e) {
    //Limpio el carousel de las amenidades
    jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
    jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
    clearTimeout(timeoutIdAmenitie);
    jQuery('.tabs-int.owl-carousel').owlCarousel('destroy');
  ...
  // jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item").hover(function () {
  //   item = jQuery(this).attr("id");
  //   //console.log(item)
  //   jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities #" + item + " > p").css("color", colorHotel)
  //   //jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities #"+item+"").append("<style>.tabs-main-amenities > #"+item+"::before{background-color: "+colorHotel+" !important;}</style>");
  // }, function () {
  //   item = jQuery(this).attr("id");
  //   jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities #" + item + " > p").css("color", "black")
  //   //jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities #"+item+"").append("<style>.tabs-main-amenities > #"+item+"::before{background-color: #dae2e3 !important;}</style>");
  // });

  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities .item').on('click', function () {
    item = jQuery(this).attr("id");
    jQuery(" section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities > .item.active").removeClass("active");
    jQuery(this).addClass("active");
    jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-main-amenities #" + item + ".active > p").css("color", "" + colorHotel + "!important")
  });

  sliderAmenities()
  amenitieContent(idAmenitie)
}


//Funcion para ver el detalle de cada amenidad asociada a un hotel
function amenitieContent(id) {
  var idsFieldAmenidad = []
  var dataAmenitie = []
  // console.log("Id seleccionado amenidad", id)
  jQuery.ajax({
    type: 'get',
    url: '/json/benefits/' + id + '',

  }).done(function (data) {
    //console.log("data amenities: ", data[0].field_amenidad)
    idsFieldAmenidad = data[0].field_amenidad;
    jQuery(idsFieldAmenidad).each(function (i, data) {

      jQuery.ajax({
        type: 'get',
        url: '/json/benefits/' + data.target_id + '',

      }).done(function (data) {
        dataAmenitie.push(data[0]);
      })
    })

  })
  timeoutIdAmenitie = setTimeout(function () {

    amentieSubItemContent(dataAmenitie);
  }, 3000);

}

function amentieSubItemContent(data) {
  console.log("Data en array", data);
  console.log("Data Images", data[0].field_galeria_imagenes);
  var title = ''
  var items = data.length
  jQuery(data).each(function (i, data) {
    if (items >= 2) {
      if (data.field_titulo_banner[0]) {
        title = data.field_titulo_banner[0].value


        if (i == 0) {
          jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').append(
            "<div class = 'item active' idAmenitie= '" + data.id[0].value + "'>" + "<p>" + title + "</p>" + "</div>"
          );
        } else {
          jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int').append(
            "<div class = 'item' idAmenitie= '" + data.id[0].value + "'>" + "<p>" + title + "</p>" + "</div>"
          );
        }
      }
  ...
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int .item.active p').css("color", colorHotel)

  if (data[0].field_titulo_banner[0]) {

    jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie").text(data[0].field_titulo_banner[0].value);
    jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie").css("color", colorHotel)
  }
  else {
    //jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie").text(data[0].id[0].value);
  }

  jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #descAmenitie").html(data[0].field_descripcion[0].processed);



  if (data[0]) {

    jQuery(data[0].field_galeria_imagenes).each(function (i, imgAmenitie) {
      var srcAmenitie = imgAmenitie.url
      //console.log("img slider amenitie", srcAmenitie)
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').append(
        "<div class = 'item'>" +
        "<img class='lazyloaded' src='" + srcAmenitie + "' alt='amenitie-image-detail'>" +
        "</div>"
      );
    })
    sliderAmenitiesDetail()
  }

  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab .owl-dots .owl-dot').append("<style>section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab .owl-dots .owl-dot.active::after{border: 1px solid " + colorHotel + " !important;}</style>");
  //jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab .owl-dots .owl-dot")
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab .owl-dots .owl-dot').css("background-color", colorHotel)

  detailAmenitiesSlider(items)
  if (items >= 5) {
    jQuery('.tabs-int.owl-carousel').on('click', '.owl-item .item', function (e) {
      jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
      // jQuery(".active").removeClass("active");
      // jQuery(this).addClass("active");
      var targetId = jQuery(this).attr("idAmenitie");
      //console.log(targetId)


      jQuery.ajax({
        type: 'get',
        url: '/json/benefits/' + targetId + '',
        //Funcion para el slider
      }).done(function (data) {
        description = data[0].field_descripcion[0].processed;
        console.log("Data images", data[0].field_galeria_imagenes)
        //console.log("Data title", data[0].field_titulo_banner)
        ...
        if (data[0].field_titulo_banner[0]) {
          ...
          // jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #titleAmenitie").text('');
        }
        jQuery(".main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .info #descAmenitie").html(description);
        ...
        //   jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').append(
        //     "<div class = 'item'>" +
        //     "<img class='lazyloaded' src='' alt='amenitie-image-detail'>" +
        //     "</div>"
        //   );
        // }
    ...
    jQuery("section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .tabs-int .item").click(function () {
      jQuery('.slider-info-tab.owl-carousel').owlCarousel('destroy');
      jQuery(".active").removeClass("active");
      jQuery(this).addClass("active");
      jQuery('.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab').text('');
      ...
      jQuery.ajax({
        ...
        if (data[0].field_galeria_imagenes) {
...
function sliderHotelBanner() {
  jQuery('.main-slider-hotel.owl-carousel').owlCarousel({
    items: 1,
    loop: true,
    margin: 0,
    nav: true,
...
function sliderAmenitiesDetail() {
  jQuery('section.main-our-resorts .wrapper .main-section-hotel .main-content-amenities .info-tab .slider-info-tab.owl-carousel').owlCarousel({
    items: 3,
    loop: true,
    margin: 0,
    center: true,
...
function sliderExperiencesDestine(items) {

  if (jQuery(window).width() > 768) {
    if (items >= 4) {

      jQuery('section.main-our-resorts .wrapper .main-content-experiences .info-tabs.owl-carousel').owlCarousel({
        items: 4,
        loop: true,
        margin: 10,
        nav: true,
  ...
  if (jQuery(window).width() < 768) {
    ...
    jQuery('section.main-our-resorts .wrapper .main-content-experiences .info-tabs').css("display", "block")
...
///Slider ADS
function sliderAds(items) {
  if (jQuery(window).width() > 768) {
    if (items >= 2) {
      jQuery('section.main-content-ads .wrapper .slider-exclusive').css("width", "100%");
      jQuery('section.main-content-ads .wrapper .slider-exclusive').css("display", "block");
      jQuery('section.main-content-ads .wrapper .slider-exclusive.owl-carousel').owlCarousel({
        items: 2,
        loop: false,
        margin: 10,
        nav: true,
...
jQuery(document).ready(function () {
  ...
  jQuery("header #block-headermenu ul li").click(function () {
    jQuery(this).find(".destinations--select").toggle()
  })

  var mainUrl = window.location.origin;
  // var mainPath = window.location.pathname;
  var urlActual = window.location.href;
  if (sessionStorage.getItem('urlActual') == undefined) {
    sessionStorage.setItem('urlActual', urlActual);
    //localStorage.setItem('data', JSON.stringify(mainData));
  } else {
    if (sessionStorage.getItem('urlAnterior') != undefined) {
      sessionStorage.removeItem('urlAnterior');
    }
    var temporal = sessionStorage.getItem("urlActual");
    sessionStorage.setItem('urlAnterior', temporal);
    sessionStorage.removeItem('urlActual');
    sessionStorage.setItem('urlActual', urlActual);
  }

  //Pregunta para poner el modal inicial. Si tengo data en el sessionStorage no debo seleccion destinos desde el modal
  if (sessionStorage.getItem('dataDestine') == undefined) {
    Modal()
  } else {
    jQuery(".initial-modal").css("display", "none");
    var dataLocalStorage = JSON.parse(sessionStorage.dataDestine)
    var allDataDestine = JSON.parse(sessionStorage.allDataDestine);
    console.log(allDataDestine)
    if (dataLocalStorage.first) {
      viewInfoDestine(dataLocalStorage.id, dataLocalStorage.subtitle)
      destinationSelect(allDataDestine)
    }
  }
  ...
  jQuery("header #block-headermenu ul .menu-icon-541 a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
    console.log(url)
    jQuery(location).attr('href', url);
  })
  jQuery("header #block-headermenu ul .menu-icon-542 a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("header #block-headermenu ul .menu-icon-543 a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("footer .wrapper nav ul .destination a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("footer .wrapper nav ul .resorts a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("footer .wrapper nav ul .packages a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("footer .wrapper nav ul .experience a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("footer .wrapper nav ul .special a").click(function (e) {
    var href = '';
    var url = '';
    href = jQuery(this).attr('href')
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("header img.img-responsive").click(function () {
    jQuery("header #block-headermenu ul.menu.menu--header-menu.nav").toggle(300);
    console.log("open menu")
  })

  jQuery("section.informative-banner .mobile-container__reserve-now").click(function () {
    jQuery("section.main-banner .wrapper .bookin").fadeIn();
  })

  jQuery("body.palace-market-us section.booking").each(function (i, value) {
    jQuery(this).addClass("book" + i + "")
  })
  jQuery("section.informative-banner .mobile-containers__reserves-now h1").click(function(e){
    //jQuery("body.palace-market-us section.book0").css("display", "block");
    jQuery("body #destination div.wrapper ").addClass("wrapper-mobile");
    jQuery("body #destination div.wrapper-mobile").removeClass("wrapper");
    jQuery("body #destination div.wrapper-mobile div.bookin").css("display", "block");
    //console.log("botón book now");
  })
  if (jQuery(window).width() < 768) {
    jQuery("body.palace-market-us section.book0").addClass("mobile");

    //console.log("Prueba")
  }else{
    jQuery("body.palace-market-us section.book0").removeClass("mobile");
    jQuery("body #destination div.wrapper").removeClass("wrapper-mobile");
    jQuery("body #destination div.wrapper-mobile").addClass("wrapper");
    //console.log("Prueba")
  }

  scrollNewlanding()

  ///Funcion para regresar de contactanos a home con la lista select
  jQuery("body.page-node-type-contact-us- header #block-headermenu ul li.first .navbar-text .destinations--select li ").click(function () {
    var href = '';
    var url = '';
    href = '#resorts'
    url = mainUrl + sessionStorage.homePath + href
  ...
  jQuery("header .menu--header-covid li.first").click(function (e) {
    //e.preventDefault();
    jQuery(".covid__pop-up").css("display", "flex");
    jQuery("body").toggleClass("modal-open");
  });



  jQuery(".covid__pop-up .close").click(function () {
    jQuery(".covid__pop-up").fadeOut(300);
    jQuery("body").toggleClass("modal-open");
  });
  // jQuery(".thank-you .close").click(function () {
  //   jQuery(".thank-you").fadeOut(300)
  // });
  jQuery("header #block-headermenu").append(jQuery(".logo"));
  // jQuery("header #block-headermenu").append(jQuery("header img.img-responsive"));

  jQuery(".page-node-type-contact-us- .webform-confirmation-modal .ui-dialog-titlebar button").click(function (e) {
    // console.log("Hice click")
    jQuery(location).prop('href', '/en/contact');
    //location.reload();
  })
  ...
  //   var tmpAc = "" + urlActual;
  //   if (tmpAn.indexOf("/discoveries/") != -1 && tmpAc.substr((tmpAc.length - 12), 12) == "/discoveries") {
  //     location.reload();
  //   }
  ...
  /* jQuery('.answers-steps .profiler-options li').click(function (e) {
  ...
  jQuery(document).on("click", ".volumeVideo", function () {
    ...
    if (activado == false) {
      ...
      if (jQuery(this).parent().attr('data-platform') == 'vimeo') {
        ...
        postMessages(jQuery(this).parent().find('iframe').attr('id'), 'unMute');
      }
      ...
      if (jQuery(this).parent().attr('data-platform') == 'vimeo') {
        ...
        postMessages(jQuery(this).parent().find('iframe').attr('id'), 'mute');
      }
  ...
  /* SliderBloque video fondo */
  if (jQuery('section.actividades_video .owl-carousel').length > 0) {
    jQuery('section.actividades_video .owl-carousel').owlCarousel({
      items: 1,
      loop: 0,
      margin: 0,
      nav: true,
  ...
  jQuery(document).ajaxSuccess(function (event, xhr, settings) {
    ...
    if (jQuery('.registration-form').length != 0) {
      //campo fecha de sección discoveries
      jQuery('form .checkin-discoveries-en').datepicker({
        autoclose: true,
        format: 'yyyy-mm-dd',
      ...
      if (jQuery(this).parent().hasClass('active') == true) {
        if (jQuery('volumeVideo:hover').length === 0 && jQuery('.main_content_banner .owl-prev:hover').length === 0 && jQuery('.main_content_banner .owl-next:hover').length === 0) {
          ...
          if (jQuery(this).attr('data-platform') == 'vimeo') {
            ...
            jQuery(this).find('.volumeVideo').removeClass('activado');
            jQuery(this).removeClass('on');
            postMessages(jQuery(this).find('iframe').attr('id'), 'mute');
          }
      ...
      if (videos != "" && videos != undefined) {
        ...
        jQuery.each(videos, function (i, v) {
          ...
          if (v.indexOf('vimeo') > -1) {
            ...
            var splitURL = splitstring[0];
            splitURL = splitURL.split('/');
            var vID = splitURL[splitURL.length - 1];
            if (onlyVideo == false) {
              var parameters = '?&autoplay=0&rel=0&loop=1&playlist=' + vID + '&showinfo=0&controls=1&enablejsapi=1';
            } else {
              var parameters = '?&autoplay=1&mute=1&rel=0&loop=1&playlist=' + vID + '&showinfo=0&controls=1&enablejsapi=1';
            }
            var script = 'https://www.youtube.com/iframe_api';
          }

          owlCarouselBanner.trigger('add.owl.carousel', ['<article class="video" data-platform="' + platform + '"><div class="volumeVideo"></div><iframe title="video" class="lazyload" data-src="' + jQuery.trim(splitstring[0]) + parameters + '" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe> <script src="' + script + '"></script></article>']);
...
jQuery(window).bind("load", function () {
  ...
  if (jQuery(window).width() <= 992) {
    ...
    jQuery("section.booking.mobile .close_h").click(function () {
      jQuery("body #destination div.wrapper-mobile div.bookin").fadeOut(300);
      jQuery("body #destination div.wrapper-mobile").addClass("wrapper");
      jQuery("body #destination div.wrapper").removeClass("wrapper-mobile");
      //jQuery(".tagove-livechat-widget.mobile-frame").fadeIn(300);
  ...
  /* const tabLinks = document.querySelectorAll(".landing-big-deals .tabs a");
  const tabPanels = document.querySelectorAll(".landing-big-deals .tabs-panel");

  for (let el of tabLinks) {
    el.addEventListener("click", e => {
      e.preventDefault();

      document.querySelector(".landing-big-deals .tabs li.active").classList.remove("active");
      document.querySelector(".landing-big-deals .tabs-panel.active").classList.remove("active");
